...
public class Tablero extends JPanel  {
	...
	public void Escribir_Tablero(int[] posicion) {
	...
	public void Derecha(int[] posicion) {
		...
		this.tablero[posicion[0]][1] = this.TURNO;
	}
	...
	public boolean Direccion_Valida(String direccion, int[] posicion) {
		...
		if( direccion.equals("A") ) {
			...
			if( posicion[0] == 0 ||								//Primera fila
				...
				System.out.println("ARRIBA " + this.TURNO);
				return true;
			}
			
			System.out.println("INCORRECTA " + this.TURNO);
			return false;
		}
			...
			if( posicion[0] == this.tablero.length - 1 ||		//Ultima fila
				...
				System.out.println("ABAJO " + this.TURNO);
				return true;	
			}
			...
			if( posicion[1] == 0 ||						 	    //Primera columna
				...
				Izquierda(posicion);
				System.out.println("IZQUIERDA " + this.TURNO);
				return true;
			}
			...
			if( posicion[1] == this.tablero.length - 1 ||		//Ultima columna
				...
				Derecha(posicion);
				System.out.println("DERECHA " + this.TURNO);
				return true;
			}
	...
	public boolean Determinar_Resultado() {
	...
	private class MouseHandler extends MouseAdapter {	
		public void mouseClicked (MouseEvent e) {
			...
			//Comprobamos que se clickeo dentro del tablero
			if( Dentro_Tablero(X, Y) ) {
			
				//Aquí irían las instrucciones para comprobar si el click del ratón se produjo en una posición correcta
				if( Posicion_Valida( posicion ) ) {
					
					//Mientras no haya ganador se siguie jugando
					if( Determinar_Resultado() ) {
